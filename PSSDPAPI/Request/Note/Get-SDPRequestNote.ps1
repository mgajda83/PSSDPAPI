...
list_info :
{
	...
	.EXAMPLE
		$Notes = Get-SDPRequestNote -RequestId 54321

	.EXAMPLE
		$Note = Get-SDPRequestNote -RequestId 54321 -NoteId 12345
	...
	Process
	{
		if($MyInvocation.BoundParameters.ContainsKey("NoteId"))
		{
			...
			#Send request
			If ($PSCmdlet.ShouldProcess($RequestId,"Get note $NoteId from request id"))
			{
			...
			#Send request
			If ($PSCmdlet.ShouldProcess($RequestId,"Get notes from request id"))
			{
				$Result = Invoke-SDPAPIEntity @InvokeParams
